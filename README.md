...
- With `--skip-existing`, a repeated exact-file handoff whose final media target already exists is treated as complete only after FFmpeg proves that the incoming and organized video/audio streams are byte-identical. MME preserves the organized copy, removes only the newly supplied duplicate, keeps existing sidecars, adds any missing same-basename sidecars from the handoff, and returns success. A different encode or unverifiable collision still fails closed without deleting or overwriting either file.
